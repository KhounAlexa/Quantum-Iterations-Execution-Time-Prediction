...
__tket_version__ = _pkg_version("pytket")
...
def calc_supermarq_features(
    ...
    depth = qc.depth(lambda x: x.operation.name not in ("barrier", "measure"))
    denom = qc.num_qubits * (qc.num_qubits - 1)
    program_communication = np.sum(connectivity) / denom if denom > 0 else 0.0

    if num_multiple_qubit_gates == 0:
        critical_depth = 0.0
    else:
        critical_depth = (
            qc.depth(filter_function=lambda x: len(x.qubits) > 1 and x.operation.name != "barrier") / num_multiple_qubit_gates
        )

    entanglement_ratio = num_multiple_qubit_gates / num_gates if num_gates > 0 else 0.0
    assert num_multiple_qubit_gates <= num_gates

    parallelism = (num_gates / depth - 1) / (qc.num_qubits - 1) if depth > 0 and qc.num_qubits > 1 else 0.0

    liveness = liveness_A_matrix / (depth * qc.num_qubits) if depth > 0 and qc.num_qubits > 0 else 0.0
