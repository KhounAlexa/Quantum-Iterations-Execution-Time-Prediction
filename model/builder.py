...
def make_model() -> nn.Module:
    ...
    stats_path = _Path(os.getcwd()) / "datav1" / "standardization_stats.pth"
    saved = _torch.load(str(stats_path), weights_only=True)
    length_of_gf = len(saved["non_zero_indices"])
    model = Simple_Model(configs.model, length_of_gf=length_of_gf)
    return model
